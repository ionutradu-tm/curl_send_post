...
RESPONSE_CODE=$(curl  --write-out %{http_code} --silent --output /tmp/respose.json -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" --data "$PAYLOAD" ${URL} )
